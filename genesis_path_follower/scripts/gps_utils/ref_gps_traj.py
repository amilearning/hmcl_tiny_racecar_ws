...
class GPSRefTrajectory():
	...
	def __init__(self, mat_filename=None, LAT0=None, LON0=None, traj_horizon=8, traj_dt=0.2):
		...
		# count =list(range(len(msg.x)))
		# # count = list(map(str, count))		
		# # for k in range(len(msg.x)):
		# # data_dict.append({'t':array(count),'lat':array(msg.x),'lon':array(msg.y),'psi':array(msg.psi)})
		# 	# count =count+ 1
		# # print(data_dict['t'])
		# tms  = count # data_dict['t']	
		# lats = msg.x # data_dict['lat']
		# lons = msg.y # data_dict['lon']
		# psis = msg.psi # data_dict['psi']
		...
		# psis  = list(psis )
		# if len(msg.x) < 33:
		# 	for i in range(0,33-len(msg.x)+1):				
		# 		tms.append(tms[-1])				
		# 		lats.append(lats[-1])
		# 		lons.append(lons[-1])
		# 		psis.append(psis[-1])
	...
	def get_waypoints(self, X_init, Y_init, psi_init, v_target=None):
		...
		if closest_index > len(self.trajectory)*0.90:
			closest_index = 0
