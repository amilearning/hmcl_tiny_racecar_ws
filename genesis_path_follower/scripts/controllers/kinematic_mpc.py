...
class KinMPCPathFollower(Controller):
	...
	def __init__(self, 
				 ...
				 DelayT     = 0, 	  # timesteps for delayed steering input, also index for free variables which becomes the actual input for the system
				 vel_lookahead_T = 2, # timesteps for velocity lookahead 
		        #  L_F        = 1.5213, # distance from CoG to front axle (m)
		        #  L_R        = 1.4987, # distance from CoG to rear axle (m)
				 L_F        = 0.125, # distance from CoG to front axle (m)
		         L_R        = 0.125, # distance from CoG to rear axle (m)
		         V_MIN      = 0.0,    # min/max velocity constraint (m/s)
		         V_MAX      = 3.0,     
		         A_MIN      = -10.0,   # min/max acceleration constraint (m/s^2)
		         A_MAX      =  10.0,     
		         DF_MIN     = -0.25,   # min/max front steer angle constraint (rad)
		         DF_MAX     =  0.25,    
		         A_DOT_MIN  = -10,   # min/max jerk constraint (m/s^3)
		         A_DOT_MAX  =  10,
		         DF_DOT_MIN = -2,   # min/max front steer angle rate constraint (rad/s)
		         DF_DOT_MAX =  2, 
		         Q = [1, 1, 10., 1], # weights on x, y, psi, and v.
	...
	def _add_constraints(self):
		...
		for i in range(self.DelayT):
			self.opti.subject_to( self.df_dv[i] == self.u_prev[1])
	...
	def solve(self):
		...
		sol_dict = {}
		sol_dict['u_control']  = u_mpc[self.DelayT,:]  # control input to apply based on solution
		sol_dict['vel_target'] = z_mpc[self.vel_lookahead_T,3]
